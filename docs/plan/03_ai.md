...
| ID | 마일스톤 | 상태 |
|---|---|---|
| M0.A1 | AI 모델·라이브러리·외부 데이터 조사 완료 | ✅ 완료 (PROGRESS_DW.md §1 21~24차·30차) |
...
| M6.A4 | **결측·이상치 처리 규칙 확정** | 컬럼별 보간 방식·IQR 임계 계수(k) — ✅ 확정 완료. `11_ai_spec.md` §5 + `docs/research/ai/02_preprocessing.md` | 처리 규칙 문서화 + spec 갱신 완료 |
...
| M6.A6 | **초기 모델 선정 + 평가 지표 확정** | 가장 균형 잡힌 모델 1개 + 보조 baseline 1개 채택. `11_ai_spec.md` §3에 라이브러리 명시. 평가 지표 산식 확정 — `11_ai_spec.md` §3 + `04_feature_spec.md` §8.2 ROI 지표 표 | 선정 사유 + 평가 결과 보고서 |
| M6.A7 | **XAI 모듈** | TreeSHAP (`shap.TreeExplainer`) 통합 — top-3 기여 피처 + 자연어 변환 여부 결정. `11_ai_spec.md` §8 확정값 활용 | SHAP 결과 시각화 + 자연어 변환 prototype |
| M6.A8 | **신뢰도 경고 기준 산정** | 신뢰도 트리거 6종 + 임계값 — ✅ 확정 완료(38차). `04_feature_spec.md` §5.3 | 임계값 산정 보고서 + spec 갱신 완료 |
...
- [docs/spec/11_ai_spec.md](../spec/11_ai_spec.md) — 본 Phase 산출물의 반영 대상
