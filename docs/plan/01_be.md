...
| M5.B3 | ARQ cron_jobs 등록 (FE 대응 없음 — BE 단독 백그라운드) | 소비기한 일일 점검(매일 **01:30** — `../spec/04_feature_spec.md` §3.6, n8n 02:00과 분산) + 단가 갱신 | 1회 트리거 → notifications INSERT |
...
> 상위: [04_gantt.md](04_gantt.md) §2 Phase 8 / §4 `n8n_data_skeleton`·`n8n_run`
...
| ID | 마일스톤 | 산출물 | 검증 |
|---|---|---|---|
| M8.B1 | `n8n_data_skeleton` — 외부 API 수집 노드 | 외부 데이터 수집 워크플로우. 확정 소스 목록: `../spec/11_ai_spec.md` §4.2 | 일일 수집 1회 통과 |
| M8.B2 | `n8n_data_skeleton` — 전처리 더미 노드 | 통과·기본 채움(NULL → 0)만. 결측 보간·이상치 탐지 로직은 M12.B1 hookup | 입력 데이터 그대로 통과 |
| M8.B3 | `n8n_run` — 야간 예측 배치 | 매일 02:00 (`../spec/04_feature_spec.md` §10.1) → AI Server `/ai/forecast/predict` → `forecast_results` INSERT → `/ai/orders/recommend` → `order_recommendations` INSERT | 1회 트리거 → 캐시 적재 |
| M8.B4 | `n8n_run` — 주간 재학습 배치 [2단계] | 매주 일요일 02:00 → AI Server `/ai/forecast/train` (`../spec/03_mvp_scope.md` §4) | 1회 트리거 통과 |
| M8.B5 | `n8n_run` — Slack 알림·재시도 | slack_sdk Webhook(개발팀 채널) + 단계별 3회 재시도. 점주 알림은 BE `NotificationService.create_and_push` 호출로 처리 (`../spec/06_sequence.md` §5) | 실패 시뮬레이션 → 재시도 + Slack 알림 |

> n8n은 `n8n_user` 계정으로 DB에 직접 쓴다. 쓰기 대상은 `pipeline_jobs`·`forecast_results`·`order_recommendations`·`order_recommendation_items`로 제한된다 (`../spec/08_schema.md` §5).

### 외부 의존

- AI 트랙 `ai_api` 완료 — AI Server 4종 엔드포인트 동작 전제
- AI 팀 결정 대기 (Phase 12 hookup): 결측 보간 방법·이상치 탐지 방법

### 참조

- [11_ai_spec.md §9 파이프라인 실행](../spec/11_ai_spec.md)
- [04_feature_spec.md §10 데이터 파이프라인](../spec/04_feature_spec.md)
- [08_schema.md §5 DB 계정 분리](../spec/08_schema.md)

### Phase 종료 조건 (M8)

`n8n_run` 완료 → **데모 시나리오 Step 3 동작** (`../spec/03_mvp_scope.md` §6 — 배치 실행)

**데모 시드:** 외부 데이터 4종 캐시 1일치, `pipeline_jobs` 성공·실패 샘플 각 1건
...
## Phase 10 쿠팡 자동 담기 — BE

> 상위: [04_gantt.md](04_gantt.md) §2 Phase 10 / §4 `auto`
> Day: 42~49 (선행: `ord_be_skeleton`)
> 브라우저 조작은 사주라 확장이 수행한다. BE는 상품 선택 판단과 결과 기록만 담당한다 — 구조 원본 `../spec/14_extension_design.md`.

### 마일스톤

| ID | 마일스톤 | 산출물 | 검증 |
|---|---|---|---|
| M10.B1 | `POST /api/orders/{order_id}/automate/match` | `AutomationService.select_products` — 확장이 보낸 품목별 검색 결과 → LLM 1회 호출 → 품목별 상품 1개 선택. 적합 상품 없으면 `selected: null` | 품목 5종 매칭 시나리오 통과 |
| M10.B2 | `POST /api/orders/{order_id}/automate/result` | `AutomationService.update_order_status` — 성공·실패 목록 기록 → `orders.status` = `AUTOMATED` / `MANUAL_REQUIRED` | 전체 성공·부분 실패·전체 실패 3종 |
| M10.B3 | LLM 클라이언트 | 제공자·모델 확정 후 추가. 키는 서버 env로만 둔다 (`../spec/14_extension_design.md` §5·§8) | 응답 스키마 검증 통과 |

> 쿠팡 자격증명 보관은 없다. 서버가 받는 데이터는 상품명·가격·용량·링크로 한정된다 (`../spec/14_extension_design.md` §4).

### 외부 의존

- FE M10.F1~F3 (확장 개발) — 확장이 보내는 payload 형태 합의 선행
- LLM 제공자 미확정 — M10.B3 착수 전 결정 필요

### 참조

- [14_extension_design.md](../spec/14_extension_design.md)
- [07_api_spec.md §7 automate/match·result](../spec/07_api_spec.md)
- [12_security.md §6 결제·자격증명](../spec/12_security.md)

### Phase 통합 종료 조건 (M10)

`auto` 완료 → **데모 시나리오 Step 6 동작** (`../spec/03_mvp_scope.md` §6 — 쿠팡 자동화)

**데모 시드:** 확장 로드된 브라우저 프로필, 품목 5종 검색 결과 샘플
...
> 배포 대상은 자체 운영 서버(Mac mini)다. GitHub Actions가 이미지를 push하고 운영 호스트가 SSH로 pull·restart한다 (`../research/backend/10_deployment.md` §4.3).

### 마일스톤

| ID | 마일스톤 | 산출물 | 검증 |
|---|---|---|---|
| M13.B1 | API 성능 검증 | k6 또는 Locust 부하 시나리오 → 응답 시간·처리량 측정 | `../spec/13_performance.md` §1.1 SLA 통과 |
| M13.B2 | 보안 검증 | Trivy(이미지) + bandit(코드) + pip-audit(의존성) + Caddy 보안 헤더 | High/Critical 0건이면 배포. spec에 SLA 정의가 없어 plan 자체 기준이다 |
| M13.B3 | CI/CD 배포 파이프라인 | GitHub Actions 8단계 (`../research/backend/10_deployment.md` §4.3) + 운영 호스트 SSH pull·restart | main push → 운영 배포 통과 |
| M13.B4 | 데모 시나리오 BE 측 검증 | Step 1~6 end-to-end API 흐름 통합 테스트 | 6단계 모두 통과 |
...
- [research/backend/10_deployment.md §4.3 CI 파이프라인](../research/backend/10_deployment.md)
...
| `check_expiry_batch` | — | `None` | **소비기한 일일 점검 — ARQ cron_jobs(매일 01:30) 진입점**. 전체 매장의 `inventory_lots.expiry_date` 조회 → D-3·D-1·초과 매칭 → `NotificationService.create_and_push` 호출하여 점주에게 인앱 + Web Push 발송 (`04_feature_spec.md` §3.6) |
...
> 알림 정책: `04_feature_spec.md` §11 / 스키마: `08_schema.md` §3.23 `notifications`·§3.24 `push_subscriptions` / 라이브러리: `docs/research/backend/06_external_integration.md` §3
