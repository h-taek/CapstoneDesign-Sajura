...
> 기능 전체 지도. 각 기능의 규칙은 아래 §1~§12에서 정의한다.

| 대분류 | 기능 | 정의 위치 |
|---|---|---|
| 인증/회원 | Google·카카오 로그인, 이메일 회원가입, 사업자 검증 게이트, 매장 정보 입력 | §1 |
| 메뉴 관리 | 메뉴 등록·조회·수정·삭제, 메뉴명 중복 검증, 레시피 관리 | §2 |
| 재고 관리 | 초기 재고 등록, 재고 조회·수정, 레시피 기반 자동 차감(FIFO), 폐기 처리, 소비기한 관리 | §3 |
| POS 연동 | CSV 업로드, POS 어댑터, 스키마 검증, 이상치 탐지 | §4 |
| 판매 데이터 | 기간별 판매 데이터 조회, 상세 조회 | §4-1 |
| 수요예측 | 1~3일 예측, 배치 예측, 예측 결과 캐싱, 신뢰도 경고 | §5 |
| 추천발주 | 품목별 권장 발주 수량, 예상 소진 시점, 점주 확인·수정·승인 | §6 |
| 발주 실행 보조 | 쿠팡 장바구니 자동 담기, 결제 안내 | §7 (확장 구조는 `14_extension_design.md`) |
...
- 단가는 초기 등록 시 쿠팡 최저가를 자동 조회하여 저장한다.
- 발주 시스템이 쿠팡에 접근할 때마다 단가를 자동 최신화한다.
- 점주가 재고 관리 화면에서 단가를 직접 수정할 수 있다.

> **현재 대체 경로 (KAMIS)** — 품목별 쿠팡 최저가 자동 조회는 재고 품목과 쿠팡 상품을 잇는 매핑 도메인(`08_schema.md` §3.12 `sites`·§3.13 `inventory_item_sites`)이 완성되어야 동작한다. 완성 전까지 품목 단가는 점주 수동 입력으로 두고, 시세 참고는 **KAMIS(한국농수산식품유통공사) 농산물유통정보 오픈API**의 주요 식자재 일별 소매가로 대신한다 — 홈 화면 노출(§12.3), 계약은 `07_api_spec.md` §5. KAMIS는 품목별 단가가 아니라 시장 시세 참고값이므로 `inventory_item_sites.last_price`에 저장하지 않는다.
...
| | 소비기한 | 선택 |
| | 단가 | 쿠팡 최저가 자동 조회 (매핑 도메인 완성 후). 현재는 수동 입력 |
| 출력 | 등록된 재고 항목 수 | |
| | 저장 완료 여부 | |
...
- 점주가 자동 담기를 요청하면 **사주라 브라우저 확장**이 점주 브라우저에서 쿠팡 장바구니에 발주 목록을 담는다.
- 서버는 담을 상품을 고르는 판단만 수행하고 쿠팡 자격증명을 보유하지 않는다.
- Backend가 담기 결과를 기록하고 발주 상태를 갱신한다.
- Frontend는 쿠팡에서 결제 진행 안내를 표시한다.

> 확장 구조·권한·데이터 범위·LLM 사용 경계 원본: `14_extension_design.md`.

### 7.1 장바구니 자동 담기

| 구분 | 항목 | 비고 |
|---|---|---|
| 입력 | 승인된 발주 내역 (품목명, 수량) | 점주 승인 후 트리거. 확장 미설치 시 담기 버튼 비활성 |
| 출력 | 장바구니 담기 성공/실패 여부 | |
| | 담기 완료 품목 목록 | |
| | 실패 품목 목록 | 실패 시 반환 |
...
> 확장 고유 실패 사유(미설치·검색 결과 0건·셀렉터 불일치): `14_extension_design.md` §7.
...
| 식자재 시세 카드 | 주요 식자재 일별 소매가·전일 대비 등락 (KAMIS — §3.1 대체 경로) |
| 알림 목록 | 최근 앱 내 알림 |
