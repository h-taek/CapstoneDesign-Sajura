...
| 쿠팡 자동화 (브라우저 확장) | 30초 이하 | 점주 브라우저에서 수행. 서버 구간은 상품 선택 1회 호출 |
...
| 컨테이너 | 예상 RSS |
|---------|-------|
| BE (Gunicorn 워커 4개) | 1.2~2 GB (쿠팡 자동화가 확장으로 이관돼 브라우저 peak 없음) |
| ARQ 워커 | ~500 MB |
| MySQL | 1.5~2 GB |
| Redis | 0.3~0.5 GB |
| n8n | 0.5~1 GB |
...
| 확장 자동 담기 불안정 | 쿠팡 웹 구조 변경 시 셀렉터 불일치 가능성 | 타임아웃 기준 적용, 실패 즉시 수동 처리 안내 (`14_extension_design.md` §7) |
