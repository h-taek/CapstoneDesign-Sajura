...
> 기능 상세 기준: 04_feature_spec.md 각 섹션
...
| 4. 수요예측 확인 | 메뉴별 1~3일 예상 수요 조회 → 예측 근거 확인 → 신뢰도 배지 확인 | 예측 근거 표시(`11_ai_spec.md` §8), 신뢰도 낮음 배지 동작 |
...
| 예측 근거 표시 | top-3 기여 요인 + 자연어 1문장 정상 표시 (`11_ai_spec.md` §8) |
...
> 13_performance.md 섹션 1.1 기준 (MVP 50개 매장·20명 동시 접속 기준)
...
| 한계 | 단일 매장 데이터라 다매장 일반화는 검증되지 않았다. 신뢰도 배지는 데이터 부족(`SHORT_HISTORY`)이 아니라 공휴일·선행일·구간 폭 트리거로 붙는다 (`04_feature_spec.md` §5.3) |

> 학습 데이터 기간·예측 오차 임계 조건은 M6.A6·M6.A8에서 확정했다 (`04_feature_spec.md` §5.3, `docs/research/ai/01_model_selection.md`).
...
| AI Modeling | 데이터 수집·전처리, 모델 학습·평가, 예측 근거 구현(`11_ai_spec.md` §8), n8n AI 파이프라인 연동 | 정동욱, 이민욱, 서창현 |
